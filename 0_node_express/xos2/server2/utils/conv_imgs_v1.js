...
//             throw new Error("Документ без картинки");
//         }
        ...
        cur.on("data", (doc) => {
            ...
            getCode(doc)
                .then((company) => {
                    ...
                    // if (Object.keys(doc).includes(fld) && doc[fld]) {
                    //     ;
                    // } else {
                    //     if (idx + 1 === def.fld.length) --count;
                    //     if (end && !count) resolve(result);
                    // }
                    // });
                    // if (idx + 1 === def.fld.length) --count;
                    // if (end && !count) resolve(result);
...
function convert(filePath, company, name, doc) {
    return new Promise((resolve, reject) => {
        ...
        const pathI = path.join(pathA, "Xostron", company, "img"); // imgPath(company,name)
...
function relocateFile(obj) {
    return new Promise((resolve, reject) => {
        ...
        const newPath = path.join(pathA, "Xclone", company, "img");
